class MarketsController < ApplicationController
  ...
  def index
    @markets = Market.all
    @upcoming_markets = Market.where('start_date_time > ?', Date.today)
    @past_markets = Market.where('end_date_time < ?', Date.today)
  end
  ...
  private
    ...
    def market_params
      params.require(:market).permit(:name, :description, :address, :start, :end)
    end
end
